...
_C=true  _E=false pe ''
_C=false _E=false pe 'jq . < initiatives/deny.definition.json'
_C=true _E=false pe 'jq . < initiatives/deny.parameters.json'
_C=true  _E=true  pe 'az policy set-definition create --name deny --display-name "Standard set of deny policies" --description "Limit regions and VM SKUs" --definitions initiatives/deny.definition.json --params initiatives/deny.parameters.json --output jsonc'
